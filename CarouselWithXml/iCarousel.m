...
#define DECELERATION_MULTIPLIER 50.0f
...
- (void)setUp
{
	...
	shouldWrap = YES;
    scrollSpeed =50;//[delegate srollSpeed:self];
    bounceDistance = 1.0f;
    toggle = 0.0f;
    stopAtItemBoundary = YES;
    scrollToItemBoundary = YES;
    useDisplayLink = YES;
    ignorePerpendicularSwipes = NO;
...
- (CGFloat)alphaForItemWithOffset:(CGFloat)offset
{
	...
	switch (iCarouselTypeInvertedWheel)
    {
    ...
    switch (iCarouselTypeWheel)
    {
        ...
        case iCarouselTypeInvertedRotary:
        {
            ...
            CGFloat arc = [self valueForTransformOption:iCarouselTranformOptionArc withDefault:M_PI * 2.0f];
             radius = [self valueForTransformOption:iCarouselTranformOptionRadius withDefault:fmaxf(itemWidth / 2.0f, itemWidth / 2.0f / tanf(arc/2.0f/count))];
        ...
        case iCarouselTypeInvertedCylinder:
        {
            ...
            CGFloat arc = [self valueForTransformOption:iCarouselTranformOptionArc withDefault:M_PI * 2.0f];
             radius = [self valueForTransformOption:iCarouselTranformOptionRadius withDefault:fmaxf(0.01f, itemWidth / 2.0f / tanf(arc/2.0f/count))];
            CGFloat angle = [self valueForTransformOption:iCarouselTranformOptionAngle withDefault:offset / count * arc];
        ...
        case iCarouselTypeInvertedWheel:
        {
            ...
            CGFloat arc = [self valueForTransformOption:iCarouselTranformOptionArc withDefault:M_PI * .8f];
             radius = [self valueForTransformOption:iCarouselTranformOptionRadius withDefault:itemWidth / arc];
      
        
         
            CGFloat angle =.65;//[delegate carouselAngle:self];
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    view.center = CGPointMake(view.bounds.size.width/2, view.bounds.size.height/2.0f);
    view.superview.center = CGPointMake(self.bounds.size.width-100,
...
{	
   ...
   [self scrollToItemAtIndex:index duration:animated? SCROLL_DURATION:2];
}
...
- (CGFloat)decelerationDistance
{
    ...
    acceleration = -startVelocity * DECELERATION_MULTIPLIER * (1.0f - decelerationRate);
...
- (void)step
{
    ...
    else if (decelerating)
    {
        ...
        CGFloat time = fminf(scrollDuration*1, currentTime - startTime);
       ...
       //acceleration=acceleration;
        ...
        [self didScroll];
        if (time == (CGFloat)scrollDuration*1)
        {
    ...
    if (scrollEnabled)
    {
        ...
        switch (panGesture.state)
        {
            case UIGestureRecognizerStateBegan:
            {
                ...
                scrolling = YES;
                decelerating = YES;
                previousTranslation =[panGesture translationInView:self].y;//: [panGesture translationInView:self].x;
            ...
            case UIGestureRecognizerStateCancelled:
            {
				...
				else if ([delegate respondsToSelector:@selector(carouselWillBeginDcarouselItemWidthecelerating:)])
				{
            ...
            default:
            {
                ...
                startVelocity = (vertical? [panGesture velocityInView:self].y: [panGesture velocityInView:self].x) * factor * scrollSpeed*5;

                    
                scrollOffset=[panGesture translationInView:self].y+temp;
